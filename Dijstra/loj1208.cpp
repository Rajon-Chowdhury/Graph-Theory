...
double dijkstra(int s,int n){
   ...
   while(!pq.empty()){
     int u=pq.top().S;
     double disx=pq.top().F;
     pq.pop();
     if(dis[u]<disx) continue;
     for(auto v:adj[u]){
       int fst=orientation(ara[u],ara[v],ara[0]);
       int sec=orientation(ara[par[u]],ara[u],ara[v]);
      // cout<<u<<" "<<v<<" "<<fst<<" "<<sec<<endl;
       if(fst>0 || sec>0) continue;


       double disy=dist(ara[u],ara[v]);
       if(disx+disy<dis[v]){
            dis[v]=disx+disy;
            pq.push(MP(dis[v],v));
            par[v]=u;
   ...
   if(orientation(ara[par[s]],ara[s],ara[x])>0) return inf;
...
int main(){
         ...
         while(t--){
            ...
            for(int i=0;i<n;i++){
                ...
                int u=mp[MP(x1,y1)],v=mp[MP(x2,y2)];
            ...
            double ans=inf;
            for(int i=1;i<=k;i++){
                double cur=dijkstra(i,k);
                ans=min(ans,cur);
            }
            for(int i=0;i<=k;i++) adj[i].clear();
            printf("Case %d: ",cs++);
            if(ans==inf) printf("-1.0\n");
            else printf("%.10f\n",ans*2);
         }
